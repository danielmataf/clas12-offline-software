...
public class RICHParticle {
    ...
    */
    ...
    // ----------------
        ...
        if (richpar.USE_PIXEL_PROPERTIES==1){
            pixel_gain  = richcal.get_PixelGain(isec, ipmt, ich);
            pixel_eff   = richcal.get_PixelEff(isec, ipmt, ich);
            pixel_flag  = richcal.get_PixelStatus(isec, ipmt, ich);
            pixel_mtime = richcal.get_PixelMTime(isec, ipmt, ich);
            pixel_stime = richcal.get_PixelSTime(isec, ipmt, ich);
        }else{
            pixel_gain  = 1.0;
            pixel_eff   = 1.0;
            pixel_flag  = 0;
            pixel_mtime = 0.0;
            pixel_stime = richpar.RICH_TIME_RMS;
        }

        if(debugMode>=1) System.out.format("Photon pixel %4d %4d %4d --> %2d %7.2f %7.2f %7.2f %7.2f \n",hit.get_id(), ipmt-1, ich-1, pixel_flag, pixel_gain, 
    ...
    */
        ...
        */
        ...
        if(richpar.USE_ELECTRON_ANGLES==1){
            for(int iref=0; iref<3; iref++)chele_emission[iref] = richcal.get_ChElectron(4, ilay_emission, ico_emission, iqua_emission, iref);
            for(int iref=0; iref<3; iref++)schele_emission[iref] = richcal.get_SChElectron(4, ilay_emission, ico_emission, iqua_emission, iref);
        }
    ...
    */
        ...
        double sigmat = richpar.RICH_TIME_RMS;
        ...
        double prob = 1 + funt*dfunt + richpar.RICH_BKG_PROBABILITY;

        if(debugMode>=1)if(prob-1>richpar.RICH_BKG_PROBABILITY)System.out.format(
    ...
    */
        ...
        double prob = 1 + pixel_eff*func*dfunc*funt*dfunt + richpar.RICH_BKG_PROBABILITY;
    ...
    */
    ...
    // ----------------
        ...
        System.out.format("       ChAngle dir (mrad) %8.2f %8.2f %8.2f %8.2f  limits %8.2f %8.2f \n", ChAngle[3][0]*MRAD, ChAngle[2][0]*MRAD, 
                                  ChAngle[1][0]*MRAD, ChAngle[0][0]*MRAD, minChAngle(0)*MRAD, maxChAngle(0)*MRAD);
        System.out.format("       ChAngle lat (mrad) %8.2f %8.2f %8.2f %8.2f  limits %8.2f %8.2f \n", ChAngle[3][1]*MRAD, ChAngle[2][1]*MRAD, 
                                  ChAngle[1][1]*MRAD, ChAngle[0][1]*MRAD, minChAngle(1)*MRAD, maxChAngle(1)*MRAD);
        System.out.format("       ChAngle spe (mrad) %8.2f %8.2f %8.2f %8.2f  limits %8.2f %8.2f \n", ChAngle[3][2]*MRAD, ChAngle[2][2]*MRAD, 
                                  ChAngle[1][2]*MRAD, ChAngle[0][2]*MRAD, minChAngle(2)*MRAD, maxChAngle(2)*MRAD);
        System.out.println("  ");
