...
public class RICHRayTrace{
    ...
    // ----------------
    public void find_EtaC_raytrace_steps(RICHParticle hadron, RICHParticle photon) {
    // ----------------
        ...
        int ntrials = 0;
        while (dist > photon.nominal_sChAngle()*100/10. && ntrials<20){
            ...
            for (nthe=1; nthe<=4; nthe++){
                ...
                double naero = 1/(hadron.get_beta(photon.get_CLASpid())*(Math.sin(Theta_P)* Math.sin(theta_dthe)*Math.cos(phi_min-Phi_P)+Math.cos(Theta_P)*Math.cos(theta_dthe)));
                ...
                if(rays_dthe!=null){
                    ...
                    if(nrefle_dthe==nrefle_min){
                        ...
                        if(debugMode>=1) System.out.format("   --> dthe pos %s  delta %7.1f (%8.2f %8.2f) \n", 
                                     pmt_dthe.toStringBrief(2), dthe*MRAD, vers_dthe.x(), vers_dthe.y());
                        break;
                    }else{
            ...
            for (int nphi=1; nphi<=4; nphi++){
                ...
                double naero = 1/(hadron.get_beta(photon.get_CLASpid())*(Math.sin(Theta_P)* Math.sin(the_min)*Math.cos(phi_dphi-Phi_P)+Math.cos(Theta_P)*Math.cos(the_min)));
                ...
                if(rays_dphi!=null){
                    ...
                    if(nrefle_dphi==nrefle_min){
                        ...
                        if(debugMode>=1) System.out.format("   --> dphi pos %s  delta %7.2f (%8.2f %8.2f) \n", 
                                     pmt_dphi.toStringBrief(2), dphi*RAD, vers_dphi.x(), vers_dphi.y());
                        break;
                    }else{
                        if(debugMode>=1) System.out.format("   failed \n");
                    }
	    ...
	    if(dthe!=0 && dphi!=0){
                ...
                for (int nn=1; nn<=4; nn++){
                    ...
                    double naero = 1/(hadron.get_beta(photon.get_CLASpid())*(Math.sin(Theta_P)* Math.sin(the_new)*Math.cos(phi_new-Phi_P)+Math.cos(Theta_P)*Math.cos(the_new)));
        ...
        if(dist < photon.nominal_sChAngle()*100.){
            ...
            if(debugMode>=1){
                photon.traced.show_raytrack();
                System.out.format("#### ETAC TRA ref %8.4f  min %8.4f  path %8.2f  time %8.2f \n", photon.EtaC_ref*MRAD, photon.traced.get_EtaC()*MRAD, photon.traced.get_path(), photon.traced.get_time());
            }
        }
