...
public class RICHio {
    ...
    // ----------------
        ...
        if(NHAD>0) {
            ...
            for(int i = 0; i < NHAD; i++){
                ...
                bankHads.setShort("hit_index",     i, (short) had.get_HitIndex());
    ...
    // ----------------
        ...
        if(NHAD>0) {
            ...
            for(int i = 0; i < NHAD; i++){
                ...
                RICHParticle had = richevent.get_Hadron(i);
                double dT_max = richpar.RICH_TIME_RMS*3;
    ...
    // ----------------
        ...
        if(NPHO!=0) {
            ...
            for(int i = 0; i < NPHO; i++){
                ...
                RICHHit hit = richevent.get_Hit( pho.get_HitIndex() );
    ...
    // ----------------
        ...
        if(NPHO!=0) {
            ...
            for(int i = 0; i < NPHO; i++){
                RICHParticle pho = richevent.get_Photon(i);
                if(pho.get_type()==0 || richpar.SAVE_THROWS==1)Nrow++;
            }
            ...
            for(int i = 0; i < NPHO; i++){
                ...
                if(pho.get_type()==0 || richpar.SAVE_THROWS==1){
    
                    if(debugMode>=1)System.out.format("   --> store \n");

                    bankPhos.setShort("id",             ientry, (short) pho.get_id());
                    bankPhos.setShort("type",           ientry, (short) pho.get_type());
                    bankPhos.setShort("hit_index",      ientry,(short) pho.get_HitIndex());
