    ...
    private void find_associate_cluster(PreCluster precluster, List<PreCluster> AHDC_precluster_list, int window,
        ...
        if(precluster.get_Super_layer() == super_layer && precluster.get_Layer() == layer && !precluster.is_Used()){
            ...
            for(PreCluster other_precluster : AHDC_precluster_list){
                if(other_precluster.get_Super_layer() == associate_super_layer && other_precluster.get_Layer() == 0
                        && !other_precluster.is_Used()){
                    double x_start = x - window;
                    double x_end = x + window;
                    double y_start = y - window;
                    double y_end = y + window;
                    if(other_precluster.get_X() > x_start && other_precluster.get_X() < x_end
                            && other_precluster.get_Y() > y_start && other_precluster.get_Y() < y_end ){
            ...
            if(possible_precluster_list.size() > 0){
                ...
                for(PreCluster possible_precluster : possible_precluster_list){
                    double distance = Math.sqrt((x - possible_precluster.get_X())*(x - possible_precluster.get_X())
                            + (y - possible_precluster.get_Y())*(y - possible_precluster.get_Y()));
                    double distance_real = Math.sqrt((precluster.get_X() - possible_precluster.get_X())*(precluster.get_X() - possible_precluster.get_X())
