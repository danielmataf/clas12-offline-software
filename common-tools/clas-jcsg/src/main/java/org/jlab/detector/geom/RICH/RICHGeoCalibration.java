...
public class RICHGeoCalibration {
    ...
    //------------------------------
        ...
        if(RICHGeoConstants.RICH_TABLE_FROM_FILE==1){
            ...
            init_RICHTableCCDB( manager.getConstants(run, "/geometry/rich/tt") );
        }
        ...
        for(int irich=1; irich<=nRICHes(); irich++){
            ...
            if(isec==0) continue;
            init_GeoCalibrationCCDB( manager.getConstants(run, "/calibration/rich/aerogel"),
    ...
    //------------------------------
        ...
        if(debugMode>=1){
            ...
            for(RICHLayerType lay: RICHLayerType.values()){
                System.out.format(" LAY %18s %10s %4d %4d %4d %10s \n", lay.name(), lay.label(), lay.id(), lay.ccdb(), lay.type(), lay.vers());
            }
            System.out.format("\n");
        }
    }
    ...
    //------------------------------
    public int find_MisaId(int ila, int ico){
    //------------------------------

        for (int im=0; im<RICHGeoConstants.NALIGN; im++){
            if(RICHGeoConstants.tool_ila[im]==ila && RICHGeoConstants.tool_ico[im]==ico) return im;
        }    
        return -1;
    }
    ...
    //------------------------------
        ...
        System.out.format("\n");
        for (int im=0; im<RICHGeoConstants.NALIGN; im++){
            int ila = RICHGeoConstants.tool_ila[im];
            int ico = RICHGeoConstants.tool_ico[im];
            int lla = RICHGeoConstants.ccdb_ila[im];
            int cco = RICHGeoConstants.ccdb_ico[im];

            if(get_AlignShift(isec, ila, ico).multiply(10.).mag()>1.e-3 || get_AlignAngle(isec, ila, ico).multiply(1000.).mag()>1.e-3){
                System.out.format("%s ALIGN   layer %4d ico %3d  (%4d %3d)  shift (mm) %s  angle (mrad) %s \n", head, lla,cco, ila,ico,
    ...
    //------------------------------
        ...
        int im = find_MisaId(ila, ico);
        if (im<0) return new Vector3D(0., 0., 0.);
    ...
    //------------------------------
        ...
        int im = find_MisaId(ila, ico);
        int irich = find_RICHModule(isec);
        if(debugMode==1)System.out.format("add_AlignShift for irich %4d isec %4d ila %4d ico %4d im %4d %s \n",
                    irich, isec, ila, ico, im, shift.toStringBrief(2));
    ...
    //------------------------------
        ...
        int im = find_MisaId(ila, ico);
        int irich = find_RICHModule(isec);
        if(debugMode==1)System.out.format("add_Alignangle for irich %4d isec %4d ila %4d ico %4d im %4d %s\n",
                    irich, isec, ila, ico, im, angle.toStringBrief(2));
